...
noticias.forEach((noticia) => {
  ...
  `;
  container.innerHTML += cardNoticia;
});

const noticiaContainer = document.querySelectorAll('div');
console.log(noticiaContainer);

//Opción2

// noticias.forEach((noticia) => {
//     const cardNoticia = ` <div class=${noticia.tipoNacional?'nacional':'internacional'}>
//     <h1>${noticia.titulo}</h1>
//     <img src=${noticia.imgUrl}>
//     <p>${noticia.descripcion}</p>
//     <span>${noticia.fecha}</span>
...
// for (const div of noticiaContainer) {
//   if (noticias.tipoNacional) {
//     div.classList.add("NA")
//   }
// }

for (let index = 0; index < noticias.length; index++) {
  if (!noticias[index].tipoNacional) {
    noticiaContainer[index].classList.add('internacional');
  }
}

// noticiaContainer.forEach()
// noticiaContainer.classList.add("card")
